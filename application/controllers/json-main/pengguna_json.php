...
class pengguna_json extends CI_Controller {
	...
	function add()
	{
		...
		$reqGroup= $this->input->post("reqGroup");

		$set= new Pengguna();
		$set->selectByParams(array());
		$set->firstRow();
		if($reqId=='' && $set->getField('user_login')==$reqUser){
			echo json_response(400, "User Login Telah Digunakan");
			exit;
		}
		...
		$set->setField("USER_LOGIN", $reqUser);
		$set->setField("USER_PASS", $reqPass);
		$set->setField("USER_GROUP_ID", $reqGroup);
