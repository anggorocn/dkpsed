...
$setTable= new Upload();
$setTable->selectByParams(array('table_nama'=>$table_nama,'table_field'=>$table_field,'table_id'=>$table_id));
                                    ...
                                    <form class="form" id="ktloginform" method="POST" enctype="multipart/form-data">
                                        <div class="w3-bar w3-black">
                                            <?for($i=0;$i<count($arrPDF); $i++){?> 
                                                <a class="w3-bar-item w3-button" onclick="openCity('table<?=$i?>')"><?=$arrPDF[$i]['keterangan']?></a>
                                            <?}?>
                                        </div>

                                        <?for($i=0;$i<count($arrPDF); $i++){?> 
                                            <div id="table<?=$i?>" class="w3-container city" <?if($i!=0){echo 'style="display:none"';}?>>
                                                <div class="card-body">
                                                    <iframe src="uploads/sample.pdf" style="width:100%; height:75vh;" frameborder="0"></iframe>
                                                </div>
                                            </div>
                                        <?}?>
