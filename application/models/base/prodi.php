...
class Prodi extends Entity{ 
	...
	function selectByParams($paramsArray=array(),$limit=-1,$from=-1, $statement='')
	{
		$str = "
		SELECT
			program_studi_id, a.nama, a.nama_lam
			, b.nama master_program_nama
			, k.nama kampus_nama
			, f.nama fakultas_nama
			, ROW_NUMBER () OVER (ORDER BY program_studi_id) as NO
		FROM program_studi A
		left join master_program b on a.master_program_id=b.master_program_id
		left join kampus k on a.kampus_id=k.kampus_id
		left join fakultas f on a.fakultas_id=f.fakultas_id
		WHERE 1=1 "; 
		...
		$str .= $statement." ORDER BY program_studi_id ASC";
		$this->query = $str;
		// echo $statement;exit;
